...
# ═══════════════════════════════════════════════════════════
    ...
    if show_all_reviews:
        st.session_state.show_all_reviews = not st.session_state.show_all_reviews

    reviews_to_show = flagged_reviews if st.session_state.show_all_reviews else flagged_reviews.head(5)

    if st.session_state.show_all_reviews:
        st.info(f"Showing all {len(flagged_reviews)} flagged reviews")

    for _, row in reviews_to_show.iterrows():
    ...
    # toggle state
    if "show_all_owners" not in st.session_state:
        st.session_state.show_all_owners = False
    if show_all_owners:
        st.session_state.show_all_owners = not st.session_state.show_all_owners

    owners_to_show = pending_owners if st.session_state.show_all_owners else pending_owners.head(5)

    if st.session_state.show_all_owners:
        st.info(f"Showing all {len(pending_owners)} pending owners")

    for _, row in owners_to_show.iterrows():
        col_info, col_act = st.columns([4, 1])
        with col_info:
            st.markdown(
